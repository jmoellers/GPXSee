...
IMG::IMG(const QString &fileName) : _file(fileName), _valid(false)
{
	...
	for (int i = 0; i < cnt; i++) {
		...
		if (tt == SubFile::TRE || tt == SubFile::LBL || tt == SubFile::RGN) {
			VectorTile *tile;
			TileMap::iterator it = _tileMap.find(fn);
			if (it == _tileMap.end()) {
				tile = new VectorTile(this);
				_tileMap.insert(fn, tile);
			} else
				tile = *it;

			SubFile *file = part ? tile->file(tt)
			  : tile->addFile(tt, size);
			CHECK(file);
	...
	QSet<int> bits;
	for (TileMap::iterator it = _tileMap.begin(); it != _tileMap.end(); ++it) {
		CHECK((*it)->init());
...
QByteArray IMG::readBlock(int blockNum)
{
	...
	if (!block) {
		if (!_file.seek(blockNum * _blockSize))
			return QByteArray();
...
{
	dbg.nospace() << "(";
	for (IMG::TileMap::const_iterator it = map.constBegin();
	  it != map.constEnd(); ++it)
		dbg << "(" << it.key() << ", " << *(*it) << ")";
	return dbg.space();
}
